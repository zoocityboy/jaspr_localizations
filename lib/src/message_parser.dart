...
// ICU MessageFormat parser for Jaspr localization
// Based on Flutter's message_parser.dart but adapted for Jaspr framework
...
class Node {
  ...
  Node.brace(this.positionInMessage, String this.value)
    : type = switch (value) {
        '{' => ST.openBrace,
        '}' => ST.closeBrace,
        _ => throw JasprL10nException('Provided value $value is not a brace.'),
      };
...
class Parser {
  Parser(this.messageId, this.filename, this.messageString, {this.useEscaping = false, this.logger, this.placeholders});
    ...
    while (startIndex < messageString.length) {
      ...
      if (isString) {
        if (useEscaping) {
          ...
          // single quotes escapes the syntax except when there are consecutive pair of single quotes.
        ...
        // Theoretically, we only reach this point because of unmatched single quotes
        throw JasprL10nParserException(
      ...
      switch (symbol) {
        case ST.message:
          if (tokens.isEmpty) {
            ...
            throw JasprL10nException('ICU Syntax Error.');
          }
        ...
        case ST.argType:
          if (tokens.isNotEmpty && tokens[0].type == ST.date) {
            ...
            throw JasprL10nException('ICU Syntax Error. Found unknown argument type.');
          }
        case ST.pluralExpr:
